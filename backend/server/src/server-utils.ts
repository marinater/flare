...
const getStringOptionsSetting = (settingName: string, options: string[], fallback?: string) => {
    if (process.env[settingName]) {
        if (options.includes(process.env[settingName] as string))
            return process.env[settingName] as string

        console.warn(`Env var ${settingName} is set but does not match an acceptable value`)
    }
...
const getAppSettings = (): AppSettings => {
    return {
        ...
        oauthStateTimeoutSeconds: getNumericSetting('OAUTH_STATE_TIMEOUT_SECONDS'),
        sessionStateTimeoutHours: getNumericSetting('SESSION_STATE_TIMEOUT_HOURS'),
        baseURL: getStringSetting('BASE_URL'),
        loggingLevel: getStringOptionsSetting('LOGGING_LEVEL', ['dev', 'short', 'tiny'], 'dev') as 'dev' | 'short' | 'tiny',
        databaseURL: getStringSetting('DATABASE_URL'),
        redisURL: getStringSetting('REDIS_URL'),
        port: getNumericSetting('PORT', 3000),
        nodeENV: getStringOptionsSetting('NODE_ENV', ['development', 'production'], 'development') as 'development' | 'production'
    }
}
