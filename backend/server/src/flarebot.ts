...
import { SocketForwardedMessage, SocketInitInfo, SocketManager, SocketPushMessage } from './controllers/sockets'
...
const getGuildMemberFromDiscordID = (discordID: string, guild: Discord.Guild) => {
...
const extractMessageContents = (msg: Discord.Message & { guild: { id: string } }): SocketForwardedMessage => {
	return {
		author: {
			id: msg.author.id,
...
export class FlareBot {
	...
	onHelpCommand = (message: Discord.Message) => {
		const helpEmbed = new Discord.MessageEmbed()
			...
			.addFields(
				{ name: '!link:', value: 'Run this command to link your Github account to your Discord account using OAuth' },
				{ name: '!connect:', value: 'Run this command inside a FlareBot associated server to receive those messages in VSCode' },
				{ name: '!disconnect:', value: 'Run this command inside a FlareBot associated server to disconnect your Discord account from VSCode' }
			)
	...
	onMessagePost = async (data: SocketPushMessage) => {
		...
		const guildMember = await getGuildMemberFromDiscordID(data.discordID, guild)

		if (!guildMember) return

		const channelUnknown = await getChannelFromChannelID(data.channelID)

		if (!channelUnknown || !guildMember.hasPermission('SEND_MESSAGES')) return

		if (channelUnknown.type !== 'text') return

		const channel = channelUnknown as Discord.TextChannel

		const res = await channel.send(`${guildMember.displayName}: ${data.content}`)
	...
	onSocketInit = async (discordID: string): Promise<SocketInitInfo> => {
		...
		if (guildsList === DatabaseCodes.Error || guildsList === DatabaseCodes.NoSuchElement) {
			return {
				discordID,
				guilds: [],
			}
