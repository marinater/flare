...
<div class="root">
    <div class="avatar-container">
        <img class="avatar-img" alt="user avatar" src={message.author.pfp}/>
    </div>
        ...
        <div class="attachment-container">
            {#each message.attachments as attachment}
                {#if attachment.height === null || attachment.width === null }
                    <div>
                        <a href={attachment.url}> {attachment.name || '(Unamed file attachment)'} </a>
                    </div>
                {:else}
                    <img class='attachment-image' alt={attachment.name || 'attached image'} src={attachment.url} style='height: {attachment.height}; width: {attachment.width};'/>
                {/if}
            {/each}
        </div>
    </div>
